...
public class LoggingPreparedStatementInvocationHandler extends
		LoggingStatementInvocationHandler {
	...
	LoggingPreparedStatementInvocationHandler(final int connectionId,
			final PreparedStatement statement, final String rawSql,
	...
	public Object invoke(final Object proxy, final Method method,
		...
		if (args == null || args.length == 0) {
			if (EXECUTE.equals(methodName) || EXECUTE_UPDATE.equals(methodName)) {
				...
				result = Utils.invokeUnwrapException(wrappedStatement, method,
						args);
				batchedPreparedOrNonPreparedStmtExecutions.add(paramValues
						.copy());
				return result;
			...
			if (methodName.startsWith("set")) {
				...
				if ("setNull".equals(methodName)) {
					paramValues.put((Serializable) args[0], new SqlTypedValue(
							null, ((Integer) args[1]).intValue()));
				} else if (args.length == 2 || "setDate".equals(methodName)
	...
	protected ResultSet internalExecuteQuery(final Method method,
		...
		try {
			...
			return (ResultSet) Proxy.newProxyInstance(
					LoggingPreparedStatementInvocationHandler.class
			...
			PerfLogger.logPreparedStatement(connectionId, logId, rawSql,
					paramValues, end - start,
					StatementType.PREPARED_QUERY_STMT, databaseType, exc);
		}

	}

	protected Object internalExecutePrepared(final Method method,
			final Object[] args) throws Throwable {
		final long start = System.nanoTime();
		Throwable exc = null;
		try {
			...
			final long end = System.nanoTime();
			PerfLogger.logPreparedStatement(connectionId, UUID.randomUUID(),
					rawSql, paramValues, end - start,
	...
	protected Object internalExecuteBatch(final Method method,
		...
		try {
			...
			final long end = System.nanoTime();
			PerfLogger.logPreparedBatchedStatements(connectionId, rawSql,
					batchedPreparedOrNonPreparedStmtExecutions, end - start,
					databaseType, exc);
			batchedPreparedOrNonPreparedStmtExecutions.clear();
		}
